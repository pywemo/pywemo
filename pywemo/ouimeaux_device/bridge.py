...
CAPABILITY_NAME2ID = dict((val, cap) for cap, val in CAPABILITY_ID2NAME.items())
...
class Bridge(Device):
    ...
    def __repr__(self):
        ...
        ).format(name=self.name, lights=len(self.Lights), groups=len(self.Groups))
...
class Light(LinkedDevice):
    ...
    def set_temperature(self, kelvin=2700, mireds=None, transition=0, delay=True):
...
class Group(LinkedDevice):
    ...
    def update_state(self, status):
        ...
        capabilities = status.findtext("GroupCapabilityIDs") or status.findtext(
            "CapabilityID"
        )
        currentstate = status.findtext("GroupCapabilityValues") or status.findtext(
            "CapabilityValue"
        )
